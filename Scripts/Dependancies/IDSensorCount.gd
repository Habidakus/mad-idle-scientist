...
func is_invention_hidden(game : SMS_Game) -> bool:
	return game.warehouse_count(SMS_Game.CraftedItemType.SENSOR_PACK) == 0;

func is_invention_pending(game : SMS_Game) -> Array[String]:
	var fraction : float = min(1.0, game.warehouse_count(SMS_Game.CraftedItemType.SENSOR_PACK) as float / sensor_count as float)
	var needs : Array = [[fraction, "Sensors"]]
	var total : float = 1.0
	if blueprint_cost > 0:
		total += 1.0
