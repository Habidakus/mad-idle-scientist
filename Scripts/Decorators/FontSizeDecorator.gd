...
var font : Font = null
var width_padding : float = 5
var height_padding : float = 3
...
func adjust_font_size() -> void:
	...
	if current_font_size != old_size:
		#print("%s.size=%s .text=%s .scale=%s .font_size=(%d -> %d)" % [target.name, target.size, target_text, target.scale, old_size, current_font_size])
